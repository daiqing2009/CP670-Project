...
#       extracted_data_dir=FLAGS.extracted_data_dir,
...
# Define constants
RATINGS_FILE_NAME = "music/rating2023.dat"
MOVIES_FILE_NAME = "music/music2023.dat"
RATINGS_DATA_COLUMNS = ["UserID", "MovieID", "Rating", "Timestamp"]
MOVIES_DATA_COLUMNS = ["MovieID", "Title", "Genres"]
OUTPUT_TRAINING_DATA_FILENAME = "train_music_amz2023.tfrecord"
OUTPUT_TESTING_DATA_FILENAME = "test_music_amz2023.tfrecord"
OUTPUT_MOVIE_VOCAB_FILENAME = "sorted_music_vocab.json"
OUTPUT_MOVIE_YEAR_VOCAB_FILENAME = "music_year_vocab.txt"
OUTPUT_MOVIE_GENRE_VOCAB_FILENAME = "music_genre_vocab.txt"
PAD_MOVIE_ID = 0
PAD_RATING = 0.0
...
def define_flags():
    ...
    flags.DEFINE_integer("min_rating", None, "Minimum rating of movie that will be used in training data")
    flags.DEFINE_float("train_data_fraction", 0.9, "Fraction of training data.")
...
class MusicInfo(collections.namedtuple("MusicInfo", ["movie_id", "timestamp", "rating", "title", "genres"])):
    """Data holder of basic information of a movie."""
    __slots__ = ()

    def __new__(cls, movie_id=PAD_MOVIE_ID, timestamp=0, rating=PAD_RATING, title="", genres=""):
        return super(MusicInfo, cls).__new__(cls, movie_id, timestamp, rating, title, genres)
...
def generate_movies_dict(movies_df):
    """Generates movies dictionary from movies dataframe."""
    movies_dict = {movie_id: MovieInfo(movie_id=movie_id, title=title, genres=genres) for movie_id, title, genres in movies_df.values}
    movies_dict[0] = MovieInfo()
    return movies_dict
...
def generate_movie_genres(movies_dict, movies):
    ...
    for movie in movies:
        if not movies_dict[movie.movie_id].genres:
            continue
        genres = [tf.compat.as_bytes(genre) for genre in movies_dict[movie.movie_id].genres.split("|")]
        movie_genres.extend(genres)
    return movie_genres
...
def generate_examples_from_single_timeline(timeline, movies_dict, max_context_len=100, max_context_movie_genre_len=320):
    """Generate TF examples from a single user timeline."""
    examples = []
    for label_idx in range(1, len(timeline)):
        start_idx = max(0, label_idx - max_context_len)
        ...
        context_movie_genres = generate_movie_genres(movies_dict, context)
        context_movie_genres = _pad_or_truncate_movie_feature(
            context_movie_genres, max_context_movie_genre_len, tf.compat.as_bytes(UNKNOWN_STR))
        feature = {
...
def generate_examples_from_timelines(timelines, movies_df, min_timeline_len=3, max_context_len=100, max_context_movie_genre_len=320, train_data_fraction=0.9, random_seed=None, shuffle=True):
    """Convert user timelines to tf examples."""
    examples = []
    movies_dict = generate_movies_dict(movies_df)
    progress_bar = tf.keras.utils.Progbar(len(timelines))
    for timeline in timelines.values():
        if len(timeline) < min_timeline_len:
            ...
            continue
        single_timeline_examples = generate_examples_from_single_timeline(
            timeline=timeline, movies_dict=movies_dict, max_context_len=max_context_len, max_context_movie_genre_len=max_context_movie_genre_len)
        examples.extend(single_timeline_examples)
        progress_bar.add(1)
    # Split the examples into train, test sets.
    if shuffle:
...
def generate_movie_feature_vocabs(movies_df, movie_counts, avg_ratings):
    """Generate vocabularies for movie features."""
    movie_vocab = []
    movie_genre_counter = collections.Counter()
    movie_year_counter = collections.Counter()

    avg_ratings_dict = dict(zip(avg_ratings['MovieID'], avg_ratings['avg_rating']))

    for movie_id, title, genres in movies_df.values:
        count = movie_counts.get(movie_id) or 0
        avg_rating = avg_ratings_dict.get(movie_id, 0)  # Get avg_rating, default to 0 if not found
        genre_list = genres.split("|")
        year = extract_year_from_title(title)
    ...
    # Sort movie_vocab by count in descending order
    movie_vocab.sort(key=lambda x: x['count'], reverse=True)

    movie_year_vocab = [0] + [x for x, _ in movie_year_counter.most_common()]
    movie_genre_vocab = [UNKNOWN_STR] + [x for x, _ in movie_genre_counter.most_common()]
...
def generate_datasets(extracted_data_dir, output_dir, min_timeline_length, max_context_length, max_context_movie_genre_length, min_rating=None, build_vocabs=True, train_data_fraction=0.9, train_filename=OUTPUT_TRAINING_DATA_FILENAME, test_filename=OUTPUT_TESTING_DATA_FILENAME, vocab_filename=OUTPUT_MOVIE_VOCAB_FILENAME, vocab_year_filename=OUTPUT_MOVIE_YEAR_VOCAB_FILENAME, vocab_genre_filename=OUTPUT_MOVIE_GENRE_VOCAB_FILENAME):
    """Generates train and test datasets as TFRecord, and returns stats."""
    logging.info("Reading data to dataframes.")
    ratings_df, movies_df, avg_ratings = read_data(extracted_data_dir, min_rating=min_rating)
